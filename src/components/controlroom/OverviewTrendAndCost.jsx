...
const MessageTypeBreakdown = ({ data }) => {
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl p-6">
      <h3 className="text-lg font-semibold text-gray-900 mb-4">Cost by Message Type</h3>
        ...
        {data.map((type, i) => (
          <div key={i} className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-2">
              ...
              <span className="text-sm text-gray-600 w-16 text-right">${type.cost.toFixed(2)}</span>
            </div>
          </div>
        ))}
      </div>
...
const ChannelEfficiency = ({ data }) => {
  return (
    <div className="bg-white border border-gray-200 rounded-xl p-6">
      <h3 className="text-lg font-semibold text-gray-900 mb-4">Cost Efficiency by Channel</h3>
      <div className="space-y-3">
        {data.map((channel, i) => (
...
const BudgetTracking = ({ data }) => {
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl p-6">
      <h3 className="text-lg font-semibold text-gray-900 mb-4">Monthly Budget Tracking</h3>
      ...
      <div className="grid grid-cols-3 gap-4 text-center">
        <div className="p-3 bg-blue-50 rounded-lg">
          <p className="text-lg font-bold text-blue-700">${currentSpend.toFixed(2)}</p>
          <p className="text-xs text-blue-600">Current Spend</p>
        </div>
        <div className="p-3 bg-orange-50 rounded-lg">
          <p className="text-lg font-bold text-orange-700">${projectedSpend.toFixed(2)}</p>
          <p className="text-xs text-orange-600">Projected Month</p>
        </div>
        <div className="p-3 bg-gray-50 rounded-lg">
          <p className="text-lg font-bold text-gray-700">${(monthlyBudget - currentSpend).toFixed(2)}</p>
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-6">
            ...
            <div className="grid grid-cols-3 gap-4 text-center">
              <div className="p-4 bg-blue-50 rounded-lg">
                <p className="text-2xl font-bold text-blue-700">{data.forecast.expectedHotLeads}</p>
                <p className="text-sm text-blue-600">Expected Hot Leads</p>
              </div>
              <div className="p-4 bg-green-50 rounded-lg">
                <p className="text-2xl font-bold text-green-700">{data.forecast.expectedHotRate}%</p>
                <p className="text-sm text-green-600">Predicted Hot Rate</p>
              </div>
              <div className="p-4 bg-purple-50 rounded-lg">
                <p className="text-2xl font-bold text-purple-700">${data.forecast.projectedCost}</p>
...
// Enhanced MetricCard Component with RBAC
const MetricCard = ({ title, value, subtext, trend, empty, onClick, isClickable = false, canAccessDetailed = true }) => {
  return (
    <div
      className={`p-4 rounded-xl text-center shadow relative transition-all duration-200 ${
        empty
