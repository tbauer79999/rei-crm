...
async function parseCSVFromURL(fileUrl: string): Promise<any[]> {
  try {
...
async function getTenantFieldConfig(tenantId: string) {
  ...
  const uniqueFields = data?.reduce((acc: any[], field: any) => {
...
async function getExistingLeads(tenantId: string, dedupeFields: string[]) {
  const { data, error } = await supabase
    .from('leads')
  ...
  data?.forEach((lead: any) => {
    const key = dedupeFields
      .map(field => lead[field]?.toLowerCase()?.trim() || '')
      .filter(v => v)
      .join('|');
...
async function processChunk(
  ...
  const dedupeFields = ['name', 'phone', 'email'].filter(fieldName => 
    fieldConfig.some(f => f.field_name === fieldName)
  );
  ...
  records.forEach((record, index) => {
    ...
    fieldConfig.forEach(field => {
      const value = record[field.field_label] || record[field.field_name] || '';
      mappedRecord[field.field_name] = value;
    ...
    const dedupeKey = dedupeFields
      .map(field => mappedRecord[field]?.toLowerCase()?.trim() || '')
      .filter(v => v)
      .join('|');
...
async function updateJobProgress(
  ...
  try {
    ...
    const { data: currentJob } = await supabase
      ...
      .select('successful_records, failed_records, duplicate_records')
      .eq('id', jobId)
      .single();
...
async function completeJob(jobId: string, status: 'completed' | 'failed', errorMessage?: string) {
  try {
    // Validate jobId
    if (!jobId || jobId === 'undefined' || jobId === 'null') {
    ...
    const { data: job } = await supabase
      .from('bulk_upload_jobs')
      .select('started_at, successful_records')
      .eq('id', jobId)
      .single();
    ...
    if (job?.started_at) {
      ...
      if (processingTimeSeconds > 0 && job.successful_records > 0) {
        updateData.average_records_per_minute = Math.round(
          (job.successful_records / processingTimeSeconds) * 60
        );
      }
    }
...
// Main job processor
async function processJob(jobId: string, jobData: any): Promise<void> {
  try {
...
serve(async (req) => {
  ...
  try {
    ...
    const { data: jobs, error } = await supabase
      .rpc('get_next_job_from_queue', { p_worker_id: workerId });
    
    if (error) {
      console.error('❌ Error getting job from queue:', error);
      return new Response(
        JSON.stringify({ error: 'Failed to get job from queue' }),
