...
serve(async (req) => {
  ...
  try {
    ...
    for (const campaign of campaigns) {
      ...
      console.log(`🚀 Processing campaign ${campaignId} (${name}) for NEW leads and RETRY messages...`);
      ...
      const { data: newLeads, error: leadErr } = await supabase
        ...
        .eq("campaign_id", campaignId)
        .eq("ai_sent", false)
        .limit(7); // Reduced to 7 to leave room for retry messages
      ...
      const { data: retryMessages, error: retryError } = await supabase
        ...
        .is('original_message_id', null)
        .eq('queued', false)
        .eq('leads.campaign_id', campaignId)
        .eq('leads.tenant_id', tenant_id)
      ...
      if (retryError) {
        ...
        processingErrors.push({ campaignId, error: `Failed to fetch retry messages: ${retryError.message}` });
      } else {
        ...
        for (let i = 0; i < retryMessages.length; i++) {
          ...
          try {
            ...
            if (!outreachResponse.ok) {
              ...
              await supabase
                .from('messages')
                .update({ retry_count: retryMessage.retry_count + 1 })
