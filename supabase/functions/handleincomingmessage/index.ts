...
serve(async (req)=>{
  ...
  try {
    ...
    let { data: lead, error: leadError } = await supabase.from('leads').select('id, tenant_id, name, phone, ai_conversation_enabled').eq('phone', normalizedFromPhone).maybeSingle();
    // If not found, try the original Twilio format
    if (!lead && !leadError) {
      console.log(`🔎 Trying original Twilio format: ${fromTwilio}`);
      const result = await supabase.from('leads').select('id, tenant_id, name, phone, ai_conversation_enabled').eq('phone', fromTwilio).maybeSingle();
      lead = result.data;
    ...
    if (!lead && !leadError && digitsOnly.length >= 10) {
      ...
      const result = await supabase.from('leads').select('id, tenant_id, name, phone, ai_conversation_enabled').or(`phone.eq.${tenDigits},phone.like.%${tenDigits}`).maybeSingle();
      lead = result.data;
      leadError = result.error;
    }
    ...
    if (!lead) {
      ...
      const { data: allLeads } = await supabase.from('leads').select('phone').limit(5);
    ...
    console.log('✅ Calling processmessageai with payload:', {
