...
serve(async (req) => {
  ...
  try {
    ...
    if (existingConversationError && existingConversationError.code !== 'PGRST116') { // PGRST116 means no rows found
      console.error('Error checking existing conversation:', existingConversationError);
      throw existingConversationError;
    }

    if (existingConversation) {
      return new Response(JSON.stringify({ success: true, message: 'Conversation already exists for this lead.', conversation_id: existingConversation.id }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        status: 200, // Not an error, just already initialized
      });
    }
    ...
    if (leadError) {
      ...
      return new Response(JSON.stringify({ error: 'Lead not found or error fetching lead data.' }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        status: 404,
      });
    }
    ...
    if (!leadTenantId) {
        return new Response(JSON.stringify({ error: 'Lead does not have an associated tenant ID.' }), {
            headers: { ...corsHeaders, 'Content-Type': 'application/json' },
            status: 400,
        });
    }
    ...
    const { data: newConversation, error: createConversationError } = await supabase
      ...
      .insert({
        lead_id: lead_id,
        tenant_id: leadTenantId, // Use the tenant_id from the lead
        status: 'active', // Default status
        current_lead_status: 'Cold Lead', // Initial lead status for conversation tracking
      })
      .select('id')
      .single();
    ...
    return new Response(JSON.stringify({ success: true, conversation_id: conversationId, message: 'Conversation initialized.' }), {
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      status: 200,
    });
  } catch (error) {
