  ...
  try {
    ...
    if (!lead_id || !tenant_id || !campaign_id || follow_up_stage === undefined) {
      return new Response(JSON.stringify({
        error: "tenant_id, lead_id, campaign_id, and follow_up_stage are required"
      }), {
        status: 400,
        headers: {
          ...corsHeaders,
    ...
    if (![
      ...
      2,
      3
    ].includes(follow_up_stage)) {
      return new Response(JSON.stringify({
        error: "follow_up_stage must be 1, 2, or 3"
      }), {
        status: 400,
        headers: {
          ...corsHeaders,
    ...
    if (lead.follow_up_stage !== expectedStage) {
      ...
      return new Response(JSON.stringify({
        error: `Lead is not ready for follow-up stage ${follow_up_stage}`
      }), {
        status: 400,
        headers: {
          ...corsHeaders,
    ...
    const { data: messageHistory } = await supabase.from("messages").select("direction").eq("lead_id", lead_id).eq("direction", "inbound");
    ...
    const { data: campaign, error: campaignError } = await supabase.from("campaigns").select("*").eq("id", campaign_id).eq("tenant_id", tenant_id).single();
    ...
    const { data: aiInstructions, error: aiError } = await supabase.from("platform_settings").select("value").eq("key", instructionKey).eq("tenant_id", tenant_id).single();
    if (aiError || !aiInstructions?.value) {
      console.error(`❌ Error fetching AI instructions for ${instructionKey}:`, aiError);
      return new Response(JSON.stringify({
        error: `Follow-up instruction template not configured: ${instructionKey}`
      }), {
        status: 400,
        headers: {
          ...corsHeaders,
...
`;
    ...
    const { data: fieldConfig, error: fieldError } = await supabase.from("lead_field_config").select("field_name, field_label").eq("tenant_id", tenant_id);
    if (fieldError) {
    ...
    const personalizedInstructions = injectLeadDataIntoTemplate(genericTemplate, lead, fieldConfig || []);
...
This is follow-up message #${follow_up_stage}. Generate a personalized follow-up message for this lead based on the instructions above.`;
    ...
    console.log('📋 Checking TCPA compliance for follow-up message...');
    ...
    if (tcpaCheck.shouldAdd) {
      ...
      console.log(`✅ TCPA text added to follow-up (${tcpaCheck.frequencyMode} mode): "${tcpaCheck.tcpaText}"`);
    } else {
      console.log(`ℹ️ TCPA text not required for this follow-up message`);
    }
...
// 10.1. Log sales activity for analytics
const { data: activityId, error: activityError } = await supabase
  .rpc('insert_sales_activity_with_followup', {
    p_tenant_id: tenant_id,
    p_lead_id: lead_id,
    p_activity_type: 'ai_followup',
    p_outcome: 'sent',
    p_notes: `AI Follow-up ${follow_up_stage} sent${hasEverResponded ? ' (Re-engagement)' : ' (Cold sequence)'}`,
    p_phone_number_used: phoneNumber.phone_number,
    p_activity_source: 'ai_followup_outreach',
    p_metadata: {
      follow_up_stage,
...
if (activityError) {
  console.error('❌ Error logging sales activity:', activityError);
    ...
    const { error: updateError } = await supabase.from("leads").update(updateData).eq("id", lead_id);
    if (updateError) {
